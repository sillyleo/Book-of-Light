...
(function () {
  ...
  const defaultSplashColor = '#000000';
  ...
  function setup() {
    const manifestEl = document
      .head
      .querySelector('link[rel="pwa-ios"]');
  ...
  function process(manifest, urlFactory) {
    ...
    updateThemeColorRender(/** @type {string} */
    ...
    function splashFor({
      ...
      ctx.fillStyle = backgroundIsLight
        ? 'white'
        : 'black';
      const textWidth = ctx
        .measureText(title)
      ...
      if (icon) {
        // nb: on Chrome, we need the image >=48px, use the big layout >=80dp, ideal is
        // >=128dp
  ...
  function updateThemeColorRender(color) {
    ...
    if (isSafari) {
      ...
      meta('apple-mobile-web-app-status-bar-style', themeIsLight
        ? 'black'
        : 'default');
    } else {
      // Edge PWA
      ...
      t.foregroundColor = colorToWindowsRGBA(themeIsLight
        ? 'black'
        : 'white');
  ...
  function shouldUseLightForeground(color) {
    ...
    // o me/browser/util/ColorUtils.java
  ...
  function contextForCanvas({width, height} = {
    width: 1,
    height: 1
  }) {
